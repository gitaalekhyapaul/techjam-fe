...
import { X, Smartphone, Check, ArrowLeft, CreditCard } from "lucide-react"

interface CreatorGooglePayWithdrawalProps {
  onClose: () => void
...
export function CreatorGooglePayWithdrawal({ 
  onClose, 
  onSuccess, 
  amount,
  currentBalance 
}: CreatorGooglePayWithdrawalProps) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.error("Creator Google Pay withdrawal error:", error)
      ...
      <Card className="w-full max-w-md bg-white">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-lg font-semibold">Creator Google Pay Withdrawal</CardTitle>
          </div>
          <Button variant="ghost" size="icon" onClick={onClose}>
            <X className="w-4 h-4" />
          </Button>
