...
import { PayPalData, mockApi } from "@/lib/constants"
...
export function PayPalPayment({ onClose, onSuccess, amount }: PayPalPaymentProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await mockApi.processPayPalPayment(formData, amount)
  ...
  const handleSavePaymentMethod = async () => {
    ...
    try {
      await mockApi.savePaymentMethod({
        type: "PayPal",
        lastFour: formData.email.split("@")[0].slice(-4),
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 mt-4">
              ...
              <span>Your payment is processed securely by PayPal</span>
            </div>
